...
from d_star import d_star  # Import the full D* algorithm
...
def main():
    ...
    clear_btn_color = (0xFF, 0x28, 0x00)  # red colour
    maze_btn_color = (0x00, 0xC0, 0x41)  # green colour
    search_btn_color = (0x00, 0xA8, 0xE8)  # blue colour
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            # Left mouse down (event.buttons[0])
                ...
                if start_clicked and not graph.is_wall(pos) and not graph.is_end(pos):
                    ...
                    if has_searched:
                        # Re-run the D* algorithm when start is moved
                    ...
                    if has_searched:
                        # Re-run the D* algorithm when end is moved
                        d_star(graph, graph.get_start_node(), graph.get_end_node())
        ...
        # Continuously update the D* search after a wall is placed or moved
        if has_searched:
            d_star(graph, graph.get_start_node(), graph.get_end_node())
